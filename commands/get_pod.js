...
const getPod = async (argv, banner) => {
    ...
    try {
        ...
        // env 
        const envVars = env.map(item => `${item.name}: ${item.value}`).join('\n');
        ...
        table[1].push({ content: `${chalk.bold.cyanBright('CPU (millicores)')}\n\n${cpuAsc}`, hAlign: 'center', rowSpan: 7 }, { content: `${chalk.bold('RAM (MB)')}\n\n${ramAsc}`, hAlign: 'center', rowSpan: 7 })
        table[2].push(podPhase)
